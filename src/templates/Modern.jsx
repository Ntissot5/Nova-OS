  ...
  return (
    <div style={{ background: '#ffffff', color: '#1a1a1a', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <nav className="flex items-center justify-between px-6 md:px-16 py-5 border-b border-black/5">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm text-black/40"><a href="#services">Services</a><a href="#about">À propos</a><a href="#contact">Contact</a></div>
        <button className="text-sm font-semibold text-white px-6 py-2.5 rounded-full" style={{ background: a }}><E field="hero.cta" editable={editable}>{hero?.cta || 'Contact'}</E></button>
      </nav>

      <section className="px-6 md:px-16 py-24 md:py-36">
        <div className="max-w-3xl">
          <E field="slogan" tag="p" editable={editable} className="text-sm font-semibold uppercase tracking-widest mb-4" style={{ color: a }}>{slogan}</E>
          <E field="hero.title" tag="h1" editable={editable} className="text-4xl md:text-6xl font-black tracking-tight leading-[1.05] mb-6" style={{ color: p }}>{hero?.title}</E>
          <E field="hero.subtitle" tag="p" editable={editable} className="text-lg text-black/40 max-w-xl mb-10 leading-relaxed">{hero?.subtitle}</E>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {services.map((s, i) => (
              <div key={i}>
                ...
                <E field={`services.${i}.name`} tag="h3" editable={editable} className="text-lg font-bold mb-2" style={{ color: p }}>{s.name}</E>
                <E field={`services.${i}.description`} tag="p" editable={editable} className="text-sm text-black/40 leading-relaxed mb-3">{s.description}</E>
      ...
      <section id="contact" className="px-6 md:px-16 py-16 border-t border-black/5">
        <div className="flex flex-col md:flex-row gap-12 justify-between max-w-4xl">
          {contact?.email && <div><div className="text-xs text-black/30 uppercase tracking-widest mb-2">Email</div><E field="contact.email" editable={editable} className="text-sm font-medium">{contact.email}</E></div>}
          {contact?.phone && <div><div className="text-xs text-black/30 uppercase tracking-widest mb-2">Téléphone</div><E field="contact.phone" editable={editable} className="text-sm font-medium">{contact.phone}</E></div>}
          {contact?.address && <div><div className="text-xs text-black/30 uppercase tracking-widest mb-2">Adresse</div><E field="contact.address" editable={editable} className="text-sm font-medium">{contact.address}</E></div>}
        </div>
      </section>
